...
# --- 1. SERIAL SETUP ---
try:
    port = ser.Serial('COM6', 115200, timeout=0) # timeout=0 makes it non-blocking
...
try:
    while True:
        ...
        if port and port.in_waiting > 0:
            ...
            line = port.read(port.in_waiting).decode('utf-8').upper()
...
try:
    ...
    if port: port.close()
>>>>>>> b4d640f9b11a0c7b5ca0c3da3d93d017129beba7
    plt.close()